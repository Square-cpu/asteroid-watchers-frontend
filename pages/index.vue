<template>
  <div class="landing-page">
    ...
    <div class="content-overlay">
      ...
      <h1>Watchers</h1>
      <h3>
        A simulation tool that models meteors orbiting Earth, tracking their trajectories
        as they enter the atmosphere and collide with the planet. It visualizes impacts
        in real time, showing scale, location, and effects of each collision
      </h3>
...
function createAsteroidMesh() {
  ...
  const params = { noiseAmp: 0.1, craterCount: 5, craterRadius: 0.2, craterDepth: 0.05 };
...
document.body.style.overflow = 'hidden'
  ...
  asteroidPositions.forEach(pos => {
    ...
    a.userData.rotVel = new THREE.Vector3((Math.random()-0.5)*0.02, (Math.random()-0.5)*0.02, (Math.random()-0.5)*0.02);
    asteroidMeshes.push(a);
    leftScene.add(a);
  });

  // RIGHT Scene (Terra + estrelas)
  rightScene = new THREE.Scene();
  rightCamera = new THREE.PerspectiveCamera(50,1,0.1,1000);
  rightCamera.position.set(0,0,5);
  rightScene.background = new THREE.Color(0x000000); // fundo preto também
  ...
  leftScene.add(stars.clone()); // mesma estrela na esquerda
...
.content-overlay button {
  background:white; color:black; padding:10px 25px; border:none; border-radius:20px;
  font-size:16px; font-family:"Roboto",sans-serif; font-weight:900; text-transform:uppercase;
  cursor:pointer; margin-top:10px; transition: transform 0.3s ease-in-out;
  margin-top: 25px;
}
.content-overlay button:hover {
  transform:scale(1.05); box-shadow:0 0 15px rgba(255,255,255,0.4);
}
</style>

...
function generateCraters(payload) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const depth =
      0.03 + Math.random() * 0.9 * Math.min(1, (28 - mag) / 28 + 0.2);
...
function applyPayloadToVisuals(payload) {
  ...
  // --- scale from diameter ---
  const d =
    payload.diameterMeters ??
    (payload.estimated_diameter_min && payload.estimated_diameter_max
      ? (payload.estimated_diameter_min + payload.estimated_diameter_max) / 2
      : 200);
  ...
  if (payload.rotation_period_hours) {
    rotationSpeed = clamp(
      (2 * Math.PI) / ((payload.rotation_period_hours * 3600) / 60),
      0.0004,
      0.08
    ); // convert to a visually meaningful increment
  } else {
    const velKmS =
      payload.relative_velocity_km_s ??
      (payload.relative_velocity_km_h
        ? payload.relative_velocity_km_h / 3600
        : 0);
  ...
  material.emissive = new THREE.Color(
    magBrightness * 0.35 + dangerBoost * 0.6,
    magBrightness * 0.18,
    magBrightness * 0.06
  );
  ...
  const inc =
    payload.orbital_angle ??
    payload.inclination ??
    payload.pole_orientation?.inclination ??
    payload.orbitalInclination ??
    payload.orbital_angle_deg ??
    payload.orbitalAngle ??
    0;
  ...
  createFragments(
    payload,
    Math.round(
      clamp((payload.fragmentation_probability ?? 0) * 30 + d / 200, 0, 40)
    )
  );
}
...
function createFragments(payload, count = 8) {
  ...
  for (let i = 0; i < count; i++) {
    const s = Math.max(
      0.03,
      Math.random() * 0.18 * Math.min(1, (payload.diameterMeters ?? 200) / 400)
    );
...
const MAX_ROTATION_SPEED = 0.02; // max ajust
const rotationFactor = 140; // adjust visual speed

function animate() {
  rafId = requestAnimationFrame(animate);

  const now = performance.now();
  let delta = (now - lastTime) / 1000; // delta in seconds
  lastTime = now;

  // apply maximum rotation limit
  ...
  if (fragGroup && fragmentsVisibleRef.value) {
    fragGroup.rotation.y +=
      effectiveRotationSpeed * 0.5 * delta * rotationFactor;
  }
...
const display = computed(() => {
  ...
  return {
    ...
    diameter: p.diameterMeters
      ? `${Math.round(p.diameterMeters)} m`
      : p.estimated_diameter_min
      ? `${Math.round(
          (p.estimated_diameter_min +
            (p.estimated_diameter_max || p.estimated_diameter_min)) /
            2
        )} m`
      : "—",
    velocity: p.relative_velocity_km_s
      ? `${p.relative_velocity_km_s.toFixed(2)} km/s`
      : p.relative_velocity_km_h
      ? `${Math.round(p.relative_velocity_km_h)} km/h`
      : "—",
    distance: p.miss_distance_km
      ? `${formatNumber(p.miss_distance_km)} km`
      : "—",
    ...
    inclination:
      ...
      p.inclination ??
      (p.pole_orientation
        ? `${p.pole_orientation.ra || "—"},${p.pole_orientation.dec || "—"}`
        : "—"),
    hazard: p.is_potentially_hazardous_asteroid ? "Yes" : "No",
    impactProb: p.impact_probability != null ? p.impact_probability : "—",
    impactEnergy: p.impact_energy_tnt
      ? `${formatNumber(p.impact_energy_tnt)} TNT`
      : "—",
